...
.. code-block::
    ...
    commands:
      {do_foo,do_bar}  specify a command to run
        do_foo         argparse CLI generated by scriptconfig 0.9.0
        do_bar         argparse CLI generated by scriptconfig 0.9.0
...
.. code-block::
    ...
    usage: demo_modal.py do_bar [-h] [--option1 OPTION1]

    argparse CLI generated by scriptconfig 0.9.0
...
.. code-block:: python
    ...
    >>> kwargs = {'num': 2}
    >>> config = ExampleConfig.cli(default=kwargs, argv=False)
    >>> assert config['num'] == 2
    ...
    >>> # It is possible to load only from CLI by setting argv=True
...
Answer:  This depends if you want to pass the path to that JSON file via the command line or if you have that file in memory already.  There are ways to do either. In the first case you can pass ``--config=<path-to-your-file>`` (assuming you have set the ``argv=True`` keyword arg when creating your config object e.g.: ``config = MyConfig(argv=True)``. In the second case when you create an instance of the scriptconfig object pass the ``default=<your dict>`` when creating the object: e.g. ``config = MyConfig(default=json.load(open(fpath, 'r')))``.  But the special ``--config`` ``--dump`` and ``--dumps`` CLI arg is baked into script config to make this easier.
